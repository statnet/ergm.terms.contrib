...
#  This InitErgmTerm function is for the 'maxdegree' term, a trivial modification of the 
# maxdegree term described
#  in Hunter, Goodreau, and Handcock (2011), "ergm.userterms:  A
#  Template Package for Extending statnet"
InitErgmTerm.maxdegree <- function(nw, arglist, ...) {
  a <- check.ErgmTerm(nw, arglist, directed=FALSE, bipartite=FALSE,
      varnames = c("maxdeg", "by"),
      vartypes = c("numeric", "character"),
      required = c(TRUE, FALSE),
      defaultvalues = list(NULL, NULL))
  if(length(a$maxdeg) > 1)
    stop("The argument maxdeg to maxdegree expected a vector of length ",
         "1, but received a vector of length ",length(a$maxdeg))
  if (is.null(a$by)) {
    attrflag <- 0
    nodecov <- NULL
    coef.names <- paste("maxdegree", a$maxdeg, sep="")
  } else {
    attrflag <- 1
    nodecov <- get.node.attr(nw, a$by)
    coef.names <- paste("maxdegree.", a$by, a$maxdeg, sep="")
  ...
  list(name = "maxdegree",
      ...
      inputs = c(attrflag, a$maxdeg, nodecov),
      dependence = TRUE,
