...
################################################################################
#Quadratically weighted edgewise shared partner term, where shared partners
#are defined by one of various specified types.
#
#Type codes are as follows (where (i,j) is the focal edge):
#
...
#overridden.  UTP cannot be chosen otherwise, since it won't work.
#
InitErgmTerm.qwesp<-function(nw, arglist, initialfit=FALSE, ...) {
  a <- check.ErgmTerm(nw, arglist,
                      varnames = c("alpha","beta","fixed","cutoff","type"),
                      vartypes = c("numeric","numeric","logical","numeric","character"),
                      defaultvalues = list(0, 0, FALSE, 30,"OTP"),
                      required = c(FALSE, FALSE, FALSE, FALSE))
  alpha<-a$alpha
  beta<-a$beta
  fixed<-a$fixed
  ...
  type.vec<-c("OTP","ITP","RTP","OSP","ISP")
  if(!(type%in%type.vec))
    stop("Illegal type code for qwesp; valid types are:",paste(type.vec, collapse=","))
  dname<-"esp"
  if(!is.directed(nw)){  #Always use UTP if undirected; others not safe
    type<-"UTP"
    typecode<-0
    basenam<-"qwesp"
  }else{ 
    typecode<-which(type==type.vec)
    basenam<-paste("qwesp",type,sep=".")
  }
  if(!initialfit && !fixed){ # This is a curved exponential family model
    maxesp <- min(cutoff,network.size(nw)-2)
    d <- 1:maxesp
    ...
    params=list(qwesp=NULL,qwesp.alpha=alpha, qwesp.beta=beta)
    names(params)<-c(basenam,paste(basenam,"alpha",sep="."), paste(basenam,"beta",sep="."))
    list(name=dname, coef.names=paste("esp#",d,sep=""), 
         inputs=c(typecode,d), params=params, map=map, gradient=gradient)
  }else{
    maxesp <- min(cutoff,network.size(nw)-2)
    ...
    if (initialfit && !fixed){  # First pass to get MPLE coefficient
      if(is.directed(nw))
        coef.names <- paste(dname,type,sep=".")
      else
        coef.names<-dname
    }else{ # fixed == TRUE
      if(is.directed(nw))
        coef.names <- paste(paste("qwesp",type,"fixed.",sep="."),paste(alpha,beta,sep=","), sep="")
      else
        coef.names <- paste("qwesp.fixed.",paste(alpha,beta,collapse=","), sep="")
    }
    list(name=dname, coef.names=coef.names, inputs=c(alpha,beta,typecode,maxesp))
  }
}
